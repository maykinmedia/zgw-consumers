...
def test_get_connection_check_correct_status_code(admin_client: Client, settings):
    ...
    with requests_mock.Mocker() as m:
        ...
        assertInHTML(
            connection_check_inner_html, response.render().content.decode("utf-8")
        )
...
def test_get_connection_opening_add_page(admin_client: Client, settings):
    ...
    connection_check_inner_html = (
        '<label>Connection check status code:</label><div class="readonly">None</div>'
    )
    assertInHTML(connection_check_inner_html, response.render().content.decode("utf-8"))
